...
public record ParsingAuditCursor(LocalDateTime createdAt, Long id) {

    private static final String DELIMITER = "_";

    public static ParsingAuditCursor from(ParsingAuditLog audit) {
        return new ParsingAuditCursor(audit.getCreatedAt(), audit.getId());
    }

    public static ParsingAuditCursor decode(String encoded) {
        try {
            String decoded = new String(Base64.getUrlDecoder().decode(encoded), StandardCharsets.UTF_8);
            int separator = decoded.lastIndexOf(DELIMITER);
            return new ParsingAuditCursor(LocalDateTime.parse(decoded.substring(0, separator)),
